...
Teams using an off-the-shelf library usually do one of two things. Either they adopt the defaults wholesale. And the product looks like every other PrimeNG app. Or they theme it heavily, which introduces drift the moment "theming" means one designer tweaking colors in Figma while the code preset keeps its defaults.

We were doing the second thing. Nobody had written down the rules.

So I wrote a [design.md](https://getdesign.md), a plain Markdown file that encodes the system (tokens, rules, component usage, anti-patterns) in a form humans *and* AI coding agents can read as instructions. Drop it in the repo and any coding agent (Claude, Cursor, Copilot) follows it when generating UI. It's not a component library. It's the rulebook *next to* one.

The complication with PrimeNG: its Figma library is minimal. It ships component-level tokens (button colors, input sizes) but **no primitive layer** (no standalone spacing, radius, or font-size scales). Its color system is built around a "surface" ramp and a single "primary" for actions. The surface ramp is used for both text and backgrounds. That's what gives PrimeNG apps their monochrome look. Theming PrimeNG means inventing the primitive and semantic layers yourself. And because that invention happens inside a Figma file, it rarely makes it back to the code side. That's exactly how drift starts.

## How I did it with Claude

I worked through it with Claude over a couple hours. Not because I couldn't write it alone. Talking it through surfaced decisions I'd been making implicitly.

Before writing anything, we looked at two references: PrimeNG's theming docs and [getdesign.md](https://getdesign.md)'s format conventions. Not to copy either, to avoid reinventing a convention that already existed. Then Claude used the Figma MCP to pull the actual variable definitions from our file (not a screenshot guess, the real token names and values). Three tiers:
...
I expected Tier 3 to be the worst offender. When I audited all 22 of ours, almost every one routed cleanly back to a semantic number. Less drift than I'd budgeted for. Still worth checking every single one, because this is where drift hides even when it's not obvious.
...
Primary was the other decision. PrimeNG uses a single `primary` slot for action color (buttons, links, focused states) and expects one color. Our brand has two blues, so I routed `primary` to both: `AzulProfundo` (deep navy) in light mode, `azulafi` (bright blue) in dark, auto-swapped by the theme. The darker blue goes in light mode for accessibility (bright `azulafi` on white doesn't pass AA on small text). The bright one earns its keep in dark mode, where dark surfaces give it the contrast it needs. Same role, two palettes, mode picks the right one.
