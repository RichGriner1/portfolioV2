...
Soy el único diseñador de Afi, una consultora fintech. Uno de nuestros productos, **Wealth Manager**, es una plataforma B2B para asesores patrimoniales: densa en información, tablas y formularios todo el día, usada por gente de finanzas a la que le importa más la fidelidad de sus datos que el efecto sorpresa. El frontend es Angular sobre **PrimeNG**, una librería de componentes estándar.

Los equipos que usan una librería estándar suelen hacer una de estas dos cosas. O adoptan los valores por defecto tal cual, y el producto acaba pareciéndose a cualquier otra app hecha con PrimeNG. O la tematizan a fondo, lo que introduce deriva en el momento en que «tematizar» significa un diseñador que ajusta colores en Figma mientras el preset del código conserva sus valores por defecto.
...
Antes de escribir nada, miramos dos referencias: la documentación de theming de PrimeNG y las convenciones de formato de [getdesign.md](https://getdesign.md). No para copiar ninguna de las dos, sino para no reinventar una convención que ya existía. Después Claude usó el MCP de Figma para extraer las definiciones de variables reales de nuestro archivo: no una suposición a partir de una captura, sino los nombres y valores de tokens de verdad. Tres niveles:
...
Esa estructura —de lo crudo a lo significativo a lo específico de componente— resultó ser la columna vertebral de todo el documento.
...
- **Nombra paletas, no roles.** «Primary» es ambiguo. El nombre de tu paleta no lo es, y los agentes de IA son los que con más probabilidad adivinarán mal si el nombre se queda en genérico.
...
- **Escribe los antipatrones.** Una lista de prohibiciones suele ser más accionable que una lista de recomendaciones. «No cambies el color primario a mano según el modo» es una regla más útil que «el color primario se enruta automáticamente».
